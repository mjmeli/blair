    ...
    >
      {loading && !data ? (
        <div className="flex items-center gap-2 py-6">
          ...
          <p className="text-sm text-slate-400">Analyzing {nightsAnalyzed || '21'} nights of data…</p>
        </div>
      ) : error ? (
        <div className="rounded-lg border border-red-500/30 bg-red-950/30 p-3 space-y-2">
          ...
          <button
            onClick={() => fetchRecommendation(true)}
            disabled={regenerating}
            className="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-2.5 py-1 text-[11px] font-medium text-red-200 transition hover:bg-red-600/30 disabled:opacity-50"
          >
            {regenerating ? <><Loader2 size={11} className="animate-spin" /> Retrying…</> : <><RefreshCw size={11} /> Retry</>}
          </button>
        </div>
      ) : !data ? (
        <p className="py-4 text-center text-sm text-slate-400">No recommendation available yet.</p>
      ) : (
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-emerald-500/30 bg-emerald-950/30 p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-400">Recommended window</p>
              {confidenceBadge}
            </div>
            ...
            {onApply && (
              <button
                ...
                className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg bg-emerald-600/30 px-3 py-1.5 text-xs font-medium text-emerald-200 transition hover:bg-emerald-600/50"
              >
          ...
          {data.current_vs_recommended && (
            <div className="flex items-start gap-2 rounded bg-slate-900/50 p-2">
              <ArrowRight size={12} className="mt-0.5 shrink-0 text-slate-500" />
              <p className="text-xs text-slate-300">{data.current_vs_recommended}</p>
            </div>
          )}

          {/* Reasoning */}
          {data.reasoning && (
            <p className="text-xs leading-relaxed text-slate-300">{data.reasoning}</p>
          )}

          {/* Patterns */}
          {data.observed_patterns && data.observed_patterns.length > 0 && (
            <div>
              <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-indigo-400">Patterns found</p>
          ...
          {data.expected_impact && (
            <div className="rounded bg-violet-950/30 p-2">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-violet-400">Expected impact</p>
              <p className="mt-0.5 text-xs text-violet-200">{data.expected_impact}</p>
            </div>
          )}
            ...
            <div className="rounded border border-amber-500/30 bg-amber-950/20 p-2">
              ...
              <ul className="space-y-0.5">
                {data.cautions.map((c, i) => (
                  <li key={i} className="text-[11px] text-amber-200/80">• {c}</li>
                ))}
              </ul>
            </div>
          )}

          <p className="text-[10px] text-slate-600">Based on {data.nights_analyzed} nights of data.</p>
        </div>
      )}
    </Card>
  );
